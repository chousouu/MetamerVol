...
def _clip_to_zero_one(reflectances):
    """
    clips values so reflectances are only function of zero and ones
    """
...
def _apply_SVD(S, use_SVD : bool):
    """
    applies SVD for get_mmb_points(), get_ocs_points()
    """
    S_or_U = S
    if use_SVD:
...
def get_mmb_points(metameric_color,
    ...
    mmb_extr_points : list
        returns points of metamer body's bound as list of 0 < [r,g,b] < 1 arrays
    """
    ...
    for k in sample_unit_sphere(S, sampling_resolution):  
        ...
        min_color_psi = cutils.get_colour_response(sens_psi.T, illum_psi, 1 - max_reflectance, WAVELENGTH_RANGE) / scale

        mmb_extr_points.extend([max_color_psi])
        mmb_extr_points.extend([min_color_psi])
...
def get_ocs_points(illum, sens, 
    ...
    ocs_extr_points : list
        returns points of ocs's bound as list of 0 < [r,g,b] < 1 arrays  
    """
    ...
    for k in sample_unit_sphere(S, sampling_resolution):
        ...
        min_color = cutils.get_colour_response(sens.T, illum, 1 - max_reflectance, WAVELENGTH_RANGE) / scale

        ocs_extr_points.extend([max_color])
        ocs_extr_points.extend([min_color])
...
def _distance(points, point):
    """
    NOT IMPLEMENTED, use dist() instead.
...
def metamer_error(pred_tristim, src_tristim,
...
def _get_scene_details(pred_tristim, src_tristim,
                             sens_phi, sens_psi, illum):
    """
